...
- **Styling**: Tailwind CSS (utility-first, no custom CSS unless necessary). Lime green (`lime-400`) is the primary accent color. See [Design System](./docs/DESIGN.md) for complete guidelines, colors, and component patterns.
...
**All features MUST work on mobile first.** Design for 375px-430px width, test on real devices (iOS/Android), then scale up to desktop. See [Design System](./docs/DESIGN.md#mobile-first-approach) for mobile guidelines.
...
- **Dumb components**: Keep components focused—render UI, handle user input, pass data up. Don't embed business logic.
- **Extract hooks for logic**: If a component has multiple concerns (mutation setup, state management, data fetching), move that logic to custom hooks (`src/client/hooks/`)
- **DRY with intent**: Apply DRY when code is repeated across multiple files/components. Don't extract if it's only used once—that's premature abstraction. Balance: 2+ uses → extract, 1 use → keep it local
- **Container vs. Presentational**: Keep a clear split. Containers handle data/logic, presentational components handle UI only
...
- ❌ No premature abstractions (extract when used in 2+ places, not before)
...
- ❌ No component logic bloat (mutations, data fetching, state should live in hooks)
